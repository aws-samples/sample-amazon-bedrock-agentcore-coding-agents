...
| Kiro | validation report over the deterministic contract | pytest remains the pass or fail floor |
| Codex | `deliverable/chatbot.html` | thin `fetch` plus `tools/call`, no pricing logic |
...
the same ten tests before finalization opens the PR through the connected GitHub PAT.
...
The PR author is the connected PAT owner. The authenticated console submitter is recorded
separately for audit and per-user cost. Neither fact is mislabeled as OAuth OBO exchange.
