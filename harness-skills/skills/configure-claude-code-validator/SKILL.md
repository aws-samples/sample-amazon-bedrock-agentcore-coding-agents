...
3. Write that executable as `acceptance_check` at the root of the workspace. The
   engine picks it up by that name; there is nothing to declare and no declaration
   file to read. Work out how to start the deliverable from what the builders actually
   wrote (their manifest, their documentation, the tree), because nothing hands you a
   start command.
...
engine then RUNS that check and reads its real exit code. It starts nothing itself and
knows nothing about the deliverable: starting it, waiting for it, and probing it all
live inside the check, which is the only thing that knows what "running" means for this
artifact.
