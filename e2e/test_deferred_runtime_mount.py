...
def test_deferred_create_then_attached_update_preserves_id_and_policy(role, tmp_path, monkeypatch):
    ...
    selected = [
        ...
        and node.name in ("deploy_runtime", "main", "_s3files_policy_resources")
    ]
    assignment = next(
    ...
    control.create_agent_runtime.return_value = {"agentRuntimeId": runtime_id, "agentRuntimeArn": runtime_arn}
    ...
    namespace = {
        "os": os, "json": json, "session": session,
